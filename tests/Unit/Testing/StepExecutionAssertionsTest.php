...
it('RecordingExecutor continues compensation and throws StepExecutionFailed even when a compensation job fails', function (): void {
    FailingCompensationJob::$executed = [];

    $definition = new PipelineDefinition(
        steps: [
            new StepDefinition(jobClass: TrackExecutionJobA::class, compensationJobClass: CompensateJobA::class),
            new StepDefinition(jobClass: TrackExecutionJobB::class, compensationJobClass: FailingCompensationJob::class),
            StepDefinition::fromJobClass(FailingJob::class),
        ],
    );
    ...
    expect($executor->compensationTriggered())->toBeTrue()
        ->and($executor->compensationSteps())->toBe([FailingCompensationJob::class, CompensateJobA::class]);
});
