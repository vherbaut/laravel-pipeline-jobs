...
| `beforeEach(Closure $hook)` | `static` | Enregistrer un hook par étape (sémantique append) invoqué avant chaque étape non ignorée. Signature : `Closure(StepDefinition, ?PipelineContext): void`. Voir [lifecycle-hooks-fr.md](lifecycle-hooks-fr.md). |
